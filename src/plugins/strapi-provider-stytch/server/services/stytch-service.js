...
const {
  loginOrCreateMagicLink,
  emailConfirmation,
} = require("../controllers/auth");
const { copyFile } = require("fs");

// stytch client is created in /extensions/user-permissions/strategies/user-permissions.js | create only one global instance for stytch client
// if (!client) {
...
module.exports = ({ strapi }) => {
  return {
    ...
    async user(data) {
      ...
      const userByDomainName = domainName
        ? await this.fetchUser({ domainName })
        : null;
      if (userByDomainName) {
    ...
    async sendMagicLink(email) {
      const params = {
        ...
        login_magic_link_url: strapi.config.get("stytch.stytch_magic_link_url"),
        signup_magic_link_url: strapi.config.get(
          "stytch.stytch_magic_link_url"
        ),
      };
    ...
    async walletLogin(walletAddress, signature) {
      ...
      const { session_token, user_id } =
        await client.cryptoWallets.authenticate(params);
    ...
    async emailLogin(token) {
      const { session_token, user_id } = await client.magicLinks.authenticate(
        token,
        { session_duration_minutes: 60 }
      );
    ...
    async oAuthLogin(token) {
      ...
      const { session_token, user_id } = await client.oauth.authenticate(
        params
      );
