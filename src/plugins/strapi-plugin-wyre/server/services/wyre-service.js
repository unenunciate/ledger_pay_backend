...
module.exports = ({ strapi }) => {
  return {
    ...
    async createOrder(paymentOrder, wyreProfile) {
      ...
      const { reservation } = await this.wyreCreateWalletReservation(
        paymentOrder
      );
      ...
      const response = await this.wyreCreateOrder(
        reservation,
        paymentOrder,
        wyreProfile
      );
    ...
    async wyreCreateOrder(reservationId, paymentOrder, wyreProfile) {
      ...
      const wyre_url = strapi.config.get("wyre.wyre_url");
      ...
      const options = {
        method: "POST",
        url: `${wyre_url}/v3/debitcard/process/partner`,
        headers: {
    ...
    async wyreCreateWalletReservation(paymentOrder) {
      ...
      const options = {
        method: "POST",
        url: `${wyre_url}/v3/orders/reserve`,
        headers: {
          accept: "application/json",
          "content-type": "application/json",
          authorization: bearerToken,
    ...
    async sanitizeWyreProfile(wyreProfile) {
      const wyreProfileSchema = strapi.getModel(
        "plugin::strapi-plugin-wyre.wyre-profile"
      );
      let sanitizedWyreProfile =
        await sanitize.sanitizers.defaultSanitizeOutput(wyreProfileSchema);
